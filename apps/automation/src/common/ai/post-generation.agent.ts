...
export class PostGenerationAgent implements OnModuleInit {
  ...
  private async buildSystemPrompt(channelSkill: string): Promise<string> {
    ...
    return [
      ...
      `- LENGTH — HARD LIMIT. News channels (channel-ai0-news, channel-ua-news, channel-space, channel-on-this-day, channel-movies): **MAX 500 characters of post body**, ideal 300–450. Birthday-story: 600–1100. All other channels: max 850.`,
      `- Distill to the MAIN THESIS in 2–4 short sentences. One headline fact + minimal context. CUT: background paragraphs, feature lists, multiple quotes, related stories, "Раніше…" / "Слід нагадати…" blocks. Keep numbers, dates, names from the source.`,
      `- If your draft exceeds the cap, REWRITE it shorter. Do not just truncate mid-sentence.`,
